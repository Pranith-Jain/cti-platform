    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-12 text-ink-1">
      ...
      <div>
        <h1 className="text-4xl font-serif font-bold mb-2 inline-flex items-center gap-3">
          <Globe size={28} className="text-accent" /> Cyber Threat Map
        </h1>
      ...
      {loading && !data && (
        <div className="font-mono text-sm text-ink-3 flex items-center justify-center" style={{ minHeight: 700 }}>
      ...
      {data && (
        <>
          <header className="flex flex-wrap items-baseline gap-x-4 gap-y-2 text-xs font-mono text-ink-2 mb-6">
            <span>
              <span className="text-ink-1 text-base font-bold">{data.total_ips}</span> malicious IPs
            </span>
            <span aria-hidden="true">·</span>
            <span>
              <span className="text-ink-1 text-base font-bold">{data.countries.length}</span> countries
            </span>
            <span aria-hidden="true">·</span>
          ...
          <div className="grid lg:grid-cols-[1fr_280px] gap-6">
            {/* Map */}
            <div
              className="rounded-lg border border-rule bg-surface-raised overflow-hidden relative"
            ...
            >
              <Suspense
                fallback={
                  <div className="flex items-center justify-center w-full h-full text-ink-3 font-mono text-xs gap-2">
              ...
              {hoveredAgg && (
                <div className="absolute top-3 left-3 rounded-lg bg-slate-900/90 dark:bg-slate-950/90 px-3 py-2 text-xs font-mono text-slate-100 border border-amber-400/40 max-w-[240px]">
              ...
              {hovered && !hoveredAgg && (
                <div className="absolute top-3 left-3 rounded-lg bg-slate-900/80 px-3 py-1.5 text-xs font-mono text-slate-300">
            ...
            <aside className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-accent">Top origins</h3>
              <ul className="space-y-1.5">
                {data.countries.slice(0, 15).map((c) => (
                  <li
                    key={c.countryCode}
                    className="flex items-baseline justify-between gap-3 text-sm font-mono px-3 py-1.5 rounded border border-rule bg-surface-page"
                  >
                    <span className="truncate">
                      <span className="text-ink-3 mr-2">{c.countryCode}</span>
                      <span className="text-ink-1">{c.country}</span>
                    </span>
                    <span className="text-accent font-bold shrink-0">{c.count}</span>
                  </li>
                ))}
              </ul>
            </aside>
          ...
          {data.samples.length > 0 && (
            <section className="mt-8">
              <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-accent mb-3">Recent IPs</h3>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {data.samples.slice(0, 30).map((s, i) => (
                  <Link
                    key={`${s.ip}-${i}`}
                    to={`/dfir/ioc-check?indicator=${encodeURIComponent(s.ip)}`}
                    className="block rounded border border-rule bg-surface-page px-3 py-2 hover:border-accent transition-colors"
                  >
                    <div className="font-mono text-sm text-ink-1 truncate">{s.ip}</div>
                    <div className="text-xs font-mono text-ink-3 flex items-center gap-2">
                      ...
                      <span className="text-accent ml-auto">{s.sources.join(', ')}</span>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {data.iocs_by_type && data.iocs_by_type.length > 0 && (
            <section className="mt-8 rounded-lg border border-rule bg-surface-page p-4">
              <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-accent mb-3">Per-type IOC feeds</h3>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 text-xs font-mono">
                {data.iocs_by_type.map((bucket) => {
                  ...
                  return (
                    <Link
                      key={bucket.type}
                      to={`/threatintel/${slug}`}
                      className="rounded border border-rule px-3 py-2 hover:border-accent transition-colors"
                    >
                      <div className="text-ink-1 font-serif font-semibold text-sm">{label} →</div>
                ...
                <Link
                  ...
                  className="rounded border border-rule px-3 py-2 hover:border-accent transition-colors"
                >
                  <div className="text-ink-1 font-serif font-semibold text-sm">All IOC types →</div>
                  <div className="text-[11px] text-ink-3 mt-0.5">URLs + domains + hashes in one view</div>
                </Link>
              </div>
            </section>
          )}
          ...
          <footer className="mt-8 text-xs font-mono text-ink-3">
